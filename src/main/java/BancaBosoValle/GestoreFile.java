...
public class GestoreFile {
  private static final String PATH_CARTELLA_UTENTI =
      "fileUtenti/"; // Cartella dove vengono salvati i file utente
  ...
  public static void salvaAggiornamenti(Utente utente) {
    ...
    try {
      ...
      for (int i = 0; i < utente.getInvestimenti().size(); i++) {
        ...
        writer.write(
            investimento.getImporto()
                + ","
                + investimento.getDurata()
                + ","
                + investimento.getRischio()
                + ","
  ...
  public static void salvaNuovoUtente(Utente utente) {
    ...
    try {
      ...
      writer.write(
          utente.getPassword()
              + "\n"); // Scrivi anche la password (nota: in un'applicazione reale, dovresti
      // criptarla)
      writer.write(utente.getMeseCorrente() + "\n");
      writer.write(utente.getPortafoglio().tostring() + "\n");
      writer.write(utente.getContoBancario().tostring() + "\n");
  ...
  public static Utente recuperaUtente(String username) {
    ...
    try {
      ...
      // System.out.println("Investimenti letti dal file: " + investimentiString); // Aggiungi log
      // per debugging
          ...
          if (!investimentoStr.trim().isEmpty()) {
            ...
            if (investimentoDetails.length == 4) {
              ...
              try {
                ...
                System.out.println(
                    "Investimento ripreso: " + investimento); // Log per il singolo investimento
